...
parent_dir = os.path.join(RootDir, 'utils/data')
...
def delete_stdin_files(parent_dir="scripts/stdin_files", extensions=[".e", ".o"]):
    """
    Removes files in parent_dir with extensions starting by (or being equal to) a certain character.
    Originally created to delete standard input (stdin) files, of the form python.exxxx (s. error) and python.oxxxx (s. output).
    """
    deleted = 0
    for path in os.listdir(parent_dir):
