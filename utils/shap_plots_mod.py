"""
Modified version of shap.plots.violin

Includes passing reference features to color the violin plot by.
"""
import warnings
...
def violin(shap_values, features=None, features_reference=None, feature_names=None, max_display=None, plot_type="violin",
    ...
    features_reference : numpy.array or pandas.DataFrame or list
        Matrix of feature values for a reference dataset that the feature values are
        colored relative to (# samples x # features) or a ``feature_names`` list as
        shorthand. If ``None``, then the feature values are colored relative to themselves.
    ...
    if isinstance(features_reference, pd.DataFrame):
        assert (features_reference.columns == feature_names).all(), "The reference features must match the features"
        features_reference = features_reference.values
        ...
        if features is not None:
            ...
            for pos, i in enumerate(feature_order):
                ...
                if features_reference is None:
                    ...
                    vmin, vmax, cvals = _trim_crange(values, nan_mask, values_reference=features_reference[:, i])
...
def _trim_crange(values, nan_mask, values_reference=None):
    ...
    if values_reference is None:
        ...
        vmin = np.nanpercentile(values_reference, 5)
        vmax = np.nanpercentile(values_reference, 95)
