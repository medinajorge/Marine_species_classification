...
try:
    opts, args = getopt.getopt(sys.argv[1:], "c:v:O:r:i:d:I:R:C:w:a:L:S:T:P:N:M:o:V:",
                               ["classifier=", "velocity=", "overwrite=", "split-random-state=", "as-image=", "as-image-density=", "as-image-indiv-scaling=", "remove-outliers=", "common-origin-distance=", "weather=", "min-animals=", "minlen=", "species=", "taxa=", "percentage-delete=", "N=", "mode=", "to-origin=", "v2="])
...
for s in tqdm(species):
    print(f"Running for species {s}")
    if s is not None or taxa is not None:
        print(f"Prunning data from species {s} and taxa {taxa}")
        prunning_kws = dict(species=s, taxa=taxa, percentage=percentage_delete, N=N, random_state=random_state, mode='specific')
        get_prunning_function = preprocessing.get_trajectory_deleter
    elif mode is not None:
        print(f"Prunning data from mode {mode}")
        prunning_kws = dict(mode=mode, percentage=percentage_delete, N=N, random_state=random_state)
        get_prunning_function = preprocessing.get_trajectory_deleter
    else:
        prunning_kws = {}
        get_prunning_function = None

    if common_origin_distance is None:
        common_origin_distance = not as_image

    kwargs = dict(classifier=classifier, overwrite=overwrite, random_state=random_state,
                  common_origin_distance=common_origin_distance, weather=weather, scale_by_velocity=scale_by_velocity, to_origin=to_origin,
                  as_image=as_image, as_image_density=as_image_density, as_image_indiv_scaling=as_image_indiv_scaling, remove_outliers=remove_outliers,
                  min_animals=min_animals, minlen=minlen,
                  get_prunning_function=get_prunning_function, prunning_kws=prunning_kws,
